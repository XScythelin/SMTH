...
// ESPFC custom read commands, let an external host poll the latest sensor data
constexpr uint16_t MSP2_ESPFC_RANGEFINDER  = 0x4F01;
constexpr uint16_t MSP2_ESPFC_OPTIC_FLOW   = 0x4F02;
...
void MspProcessor::processCommand(MspMessage& m, MspResponse& r, Device::SerialDevice& s)
{
  ...
  switch(m.cmd)
  {
    ...
    case MSP2_ESPFC_RANGEFINDER:
      // full rangefinder readout for an external host
      {
        const Espfc::RangefinderState& rf = _model.state.rangefinder;
  ...
  case MSP_GPSSVINFO:
    ...
    case MSP2_SENSOR_OPTIC_FLOW:
      // incoming optical flow feed from an external UART sensor (e.g. MTF-02)
      {
        ...
        Espfc::FlowState& f = _model.state.flow;
        f.quality = quality;
        f.motionX = motionX;
        f.motionY = motionY;
        f.lastMsgTs = micros();
